...
const OPEN_OFFSET =
  OPTIONS_X - ACTION_WIDTH / 2 - ACTION_GAP - NOTIFICATION_WIDTH / 2;
...
function getActionOpacity(actionX: number, notificationOffset: number) {
  ...
  const disappearedFraction = clamp(
    (notificationRightEdge - actionLeftEdge) / ACTION_WIDTH,
    0,
    1,
  );
...
function NotificationScene({ nightMode }: { nightMode: boolean }) {
  ...
  function updateActionLabels(notificationOffset: number) {
    ...
    if (clearLabelRef.current) {
      clearLabelRef.current.style.opacity = String(
        getActionOpacity(CLEAR_X, notificationOffset),
      );
    }
  }
  ...
  const bind = useDrag(
    ({ active, first, last, offset: [notificationOffset] }) => {
      ...
      if (last || !active) {
        ...
        setCanvasCursor(
          notificationHoveredRef.current ? styles.canvasGrab : null,
        );
        animateNotificationOffset(
          boundedOffset < OPEN_OFFSET * 0.42 ? OPEN_OFFSET : 0,
        );
        return;
      }
  ...
  return (
    <ZStack alignment="center">
      <Html zIndex={-2} sizing="fill">
        <div
          className={`${styles.backdrop} ${nightMode ? styles.backdropNight : ""}`}
        >
          <img
            alt=""
      ...
      <Frame maxWidth={Infinity} maxHeight={Infinity}>
        <GlassContainer
          ...
          tint={
            nightMode
              ? { r: 0.7, g: 0.7, b: 0.7, a: 0.22 }
              : { r: 0.82, g: 0.92, b: 0.95, a: 0.22 }
          }
    ...
    <div className={styles.content}>
      ...
      <div className={styles.copy}>
        ...
        <p className={styles.message}>
          My orchard just shipped a critical bug fix. The apples now fall closer
          to the tree.
        </p>
      </div>
    </div>
