...
function RainbowCubeScene({ controls }: { controls: ControlsState }) {
  return (
    <>
      ...
      <directionalLight color="#fff6dc" intensity={2} position={[2.5, 3.5, 5]} />
      <pointLight color="#fff0a8" distance={7} intensity={12} position={[-2.8, 1.4, 3.2]} />
      <pointLight color="#ff8a4d" distance={8} intensity={10} position={[3.2, -1.4, 2.5]} />
...
function RainbowCube({
  ...
  return (
    <mesh ref={ref} position={position} rotation={[phase * 0.16, phase * 0.21, phase * 0.08]}>
...
function ControlsForm({
  ...
  return (
    <form className={styles.controlsPanel} onSubmit={(event) => event.preventDefault()}>
      <RangeControl
        label="Cube size"
        min={0.22}
        max={0.72}
